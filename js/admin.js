...
function previewModalPhoto(input) {
  ...
  if (file.size > 1024 * 1024) {
    input.value = '';
    const wrap = input.closest('.photo-upload');
    if (wrap) {
      wrap.style.borderColor = 'var(--red)';
      const warn = document.createElement('span');
      warn.style.cssText = 'font-size:11px;color:var(--red);padding:0 0.75rem;pointer-events:none;position:absolute;z-index:3;';
      warn.textContent = 'حجم الصورة يتجاوز 1MB';
      wrap.appendChild(warn);
      setTimeout(() => { wrap.style.borderColor = ''; warn.remove(); }, 3500);
    }
    return;
  }
