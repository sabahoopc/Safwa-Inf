...
function previewPhoto(input) {
  ...
  if (file.size > 1024 * 1024) {
    ...
    if (zone) {
      ...
      const warn = zone.querySelector('.size-warn') || document.createElement('span');
      warn.className = 'size-warn';
      warn.style.cssText = 'font-size:12px;color:var(--red);padding:0 1rem;pointer-events:none;';
      warn.textContent = 'حجم الصورة يتجاوز 1MB — اختر صورة أصغر';
      if (!zone.querySelector('.size-warn')) zone.appendChild(warn);
      setTimeout(() => { zone.style.borderColor = ''; warn.remove(); }, 3500);
    }
    return;
  }
