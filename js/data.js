/**
 * data.js — Firebase Firestore integration
 * يستبدل localStorage بقاعدة بيانات مركزية حقيقية
 */
...
// ─── Seed Data (يُضاف مرة واحدة فقط عند أول تشغيل) ──
...
async function addRemoval(data) {
  // increment removalCount on the influencer
  const q    = query(collection(db, INFS_COL), where("name", "==", data.target), where("status", "==", "active"));
  const snap = await getDocs(q);
  if (!snap.empty) {
    const infDoc = snap.docs[0];
    const current = infDoc.data().removalCount || 0;
    await updateDoc(doc(db, INFS_COL, infDoc.id), { removalCount: current + 1 });
  }
...
export {
  ...
  pickColor, getInitials, genId, relativeDate, ADMIN_PASS
};
